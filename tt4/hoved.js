...
function lavxmlserver() {
    tildeltid0();
    var senderobjekt = [];
    var eventsderbliversendt = [];
    var medtag = ["id", "preasigntime", "duration", "eventresmangler"];
    senderobjekt.push(events);
    var stringified = JSON.stringify(senderobjekt, medtag);

    $.ajax({
        type: "POST",
        url: "Default.aspx/savesolution",
        data: "{sol:" + stringified + "}",
...
function lavxml(server) {
    ...
    if (!server)
        window.open('data:text/xml,' + serializer.serializeToString(xmlDoc)); else {
        $.ajax({
            type: "POST",
            url: "Default.aspx/savesolution",
            data: "{sol:" + JSON.stringify((xmlDoc)) + "}",
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function (response) {
                var k = response.d;
...
function choicemade(tidangivet, mangelindex, dropdown) {
    ...
    if (tidangivet) {
        ...
        if (tidmangelindex < 0)
            var tidindex = tidmangelindex * (-1) - 1; else
        ...
        if (isNaN(nyval))
            vistsol.resmangeltildelinger[mangelindex] = null; else
        ...
        for (var durationindex = 0; durationindex < ievent.duration; durationindex++) {
            ...
            if (ievent.preasigntime) {
                if (durationindex == 0) {
                    ...
                    if (tooltip.length > 0)
                        htmltxt += "<tr title='" + tooltip.substr(0, tooltip.length - 2) + "'><td>"; else
                        htmltxt += "<tr><td>";
            ...
            for (var mnglindex = fra; mnglindex < antalmngler; mnglindex = mnglindex + countplusser) {
                ...
                if (ievent.eventtidmangler[durationindex] != undefined)
                    var tidmangelindex = ievent.eventtidmangler[durationindex].index; else
